...
namespace driver {
	...
	NTSTATUS device_control(PDEVICE_OBJECT device_object, PIRP irp) {
		...
		switch (control_code) {
			...
			case codes::write:
				if (target_process != nullptr) {
					// Copy memory into target process
...
NTSTATUS driver_main(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	RtlInitUnicodeString(&device_name, L"\\Device\\BunnyDriver");
	...
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\BunnyDriver");
	...
	// Allow sending small amounts of data between user mode and kernel mode
...
NTSTATUS DriverEntry() {
	...
	RtlInitUnicodeString(&driver_name, L"\\Driver\\BunnyDriver");
